...
description: A warm-canvas editorial interface for AegisScan. The system anchors on a tinted cream canvas with serif display headlines, dusty-burgundy CTAs, and dark product surfaces (code editor mockups, model showcase cards). Brand voltage comes from the cream/burgundy pairing — deliberately warm and literary where most security products use cool blue + slate. Type voice runs a slab-serif display ("Copernicus" / Tiempos Headline) for h1/h2 and a humanist sans for body. A black radial-spike mark anchors the wordmark.

colors:
  primary: "#8b4651"
  primary-active: "#6f3540"
...
Brand voltage comes from the **cream + dusty-burgundy pairing** — burgundy (`{colors.primary}` — #8b4651) is the signature AegisScan accent, used on every primary CTA, on the brand wordmark, and on full-bleed callout cards. The burgundy is muted, literary, and never cyan/blue — a deliberate counter-positioning against the security category's cool slate and corporate cyan.
...
- Dusty-burgundy primary CTA (`{colors.primary}` — #8b4651). Used scarcely on individual buttons, generously on full-bleed burgundy callout cards.
...
### Brand & Accent
- **Dusty Burgundy / Primary** (`{colors.primary}` — #8b4651): The signature AegisScan accent. Used on every primary CTA background, on full-bleed burgundy callout cards, and on the brand wordmark accent.
- **Burgundy Active** (`{colors.primary-active}` — #6f3540): The press / darker variant.
- **Burgundy Disabled** (`{colors.primary-disabled}` — #e6dfd8): A desaturated cream-tinted disabled state.
...
- **Callout / CTA bands:** `{spacing.xxl}` (48px) inside burgundy callout cards; 64px inside the larger dark CTA band.
...
- Some hero illustrations use simple line-art with burgundy and dark-surface strokes on cream — minimal, hand-drawn-feeling, never photorealistic.
...
- Simple line-art illustrations with burgundy + dark-surface strokes on the cream canvas
...
**`top-nav`** — Cream nav bar pinned to the top of every page. 64px tall, `{colors.canvas}` background. Carries the spike-mark + AegisScan wordmark at left, navigation center-left, and the primary `{component.button-primary}` action at right. Menu items use `{typography.nav-link}` (StyreneB 14px / 500).

### Buttons

**`button-primary`** — The signature dusty-burgundy CTA. Background `{colors.primary}` (#8b4651), text `{colors.on-primary}` (white), type `{typography.button}` (StyreneB 14px / 500), padding 12px × 20px, height 40px, rounded `{rounded.md}` (8px). Active state `button-primary-active` darkens to `{colors.primary-active}` (#6f3540).
...
**`text-link`** — Inline body links in `{colors.primary}` (the burgundy). Underlined on press; the burgundy inline link is one of the system's most distinctive small details.
...
**`hero-illustration-card`** — A larger card holding the hero's right-side artifact — sometimes a burgundy-stroke line illustration on cream background, sometimes a dark code editor mockup. Background `{colors.canvas}` or `{colors.surface-dark}` depending on context, rounded `{rounded.xl}` (16px).
...
**`callout-card-burgundy`** — A full-bleed burgundy card carrying a major call-to-action. Background `{colors.primary}` (#8b4651), text `{colors.on-primary}` (white), rounded `{rounded.lg}`, padding `{spacing.xxl}` (48px). The burgundy surface provides the voltage; the CTA inside uses an inverted cream/canvas button.
...
**`text-input-focused`** — Focus state. Border thickens or shifts to `{colors.primary}` (burgundy) for emphasis. Carries a 3px burgundy-at-15%-alpha outer ring.
...
**`badge-burgundy`** — Burgundy-fill badge for "NEW", "BETA", featured highlights. Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.caption-uppercase}` (12px / 500 / 1.5px tracking), rounded `{rounded.pill}`, padding 4px × 12px.
...
**`cta-band-burgundy`** — A pre-footer CTA card. Full-width burgundy fill, white type, rounded `{rounded.lg}`, padding 64px. Carries an h2 in `{typography.display-sm}` (still serif!), a sub-line, and a cream-button CTA.
...
- Reserve `{colors.primary}` (burgundy) for primary CTAs and full-bleed `{component.callout-card-burgundy}` moments. Don't paint accent moments burgundy elsewhere.
...
- Don't use cool blue or saturated cyan as a brand accent. Dusty burgundy is the brand voltage.
- Don't put burgundy everywhere. Burgundy is scarce on individual elements and generous only on full-bleed callout cards.
- Don't use Inter for display headlines. The serif character is the brand voice.
- Don't repeat the same surface mode in two consecutive bands. The pacing alternates: cream → cream-card → dark-mockup → cream → burgundy-callout → dark-footer.
...
6. Cream + dusty burgundy + dark surface is the trinity. Don't introduce a fourth surface tone (no purple cards, no green sections).
